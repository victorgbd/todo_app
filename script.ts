...
class TodoList {
    ...
    private filter: 'todas' | 'activas' | 'completas' = 'todas';
    ...
    private loadFromLocalStorage(): void {
        ...
        if (todoItemsJson) {
            try {
                this.todoItems = JSON.parse(todoItemsJson) as TodoItem[];
            } catch (e) {
    ...
    private renderList(): void {
        ...
        if (todasList && activasList && completasList) {
            ...
            const renderItems = (listElement: HTMLElement, items: TodoItem[]) => {
                items.forEach((item) => {
    ...
    private togglecompletas(id: string): void {
    ...
    public addItem(): void {
        const newItemInput = document.getElementById('form2') as HTMLInputElement;
    ...
    public setFilter(filter: 'todas' | 'activas' | 'completas'): void {
        this.filter = filter;
        this.renderList();
    }
